...
export class Gulpfile {
  ...
  @Task()
  compile() {
    return gulp.src('package.json', { read: false }).pipe(shell(['"node_modules/.bin/ngc" -p tsconfig-aot.json']));
  }
  ...
  compileBundles() {
    ...
    return [
      gulp
        .src('build/bundle/**/*.ts')
        .pipe(amdTsProject())
        .js.pipe(gulp.dest('build/package')),

      gulp
        .src('build/bundle/**/*.ts')
        .pipe(systemTsProject())
        .js.pipe(gulp.dest('build/package')),

      gulp
        .src('build/bundle/**/*.ts')
        .pipe(amdPureTsProject())
        .js.pipe(gulp.dest('build/package')),

      gulp
        .src('build/bundle/**/*.ts')
        .pipe(systemPureTsProject())
        .js.pipe(gulp.dest('build/package'))
    ];
  }
  ...
  @Task()
  bundleCopySources() {
    return gulp.src(['./src/**/*.ts']).pipe(gulp.dest('./build/bundle/' + packageName));
  }
  ...
  uglify() {
    return [
      gulp
        .src(`./build/package/${packageName}.pure.amd.js`)
        .pipe(uglify())
        .pipe(rename(`${packageName}.pure.amd.min.js`))
        .pipe(gulp.dest('./build/package')),

      gulp
        .src(`./build/package/${packageName}.pure.system.js`)
        .pipe(uglify())
        .pipe(rename(`${packageName}.pure.system.min.js`))
        .pipe(gulp.dest('./build/package')),

      gulp
        .src(`./build/package/${packageName}.amd.js`)
        .pipe(uglify())
        .pipe(rename(`${packageName}.amd.min.js`))
        .pipe(gulp.dest('./build/package')),

      gulp
        .src(`./build/package/${packageName}.system.js`)
        .pipe(uglify())
  ...
  unit() {
    ...
    return gulp.src('./build/es5/test/unit/**/*.js').pipe(mocha());
  }
